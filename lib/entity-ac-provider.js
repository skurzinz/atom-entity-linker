...
const API_URL = 'https://mpr.acdh.oeaw.ac.at/apis/tei/autocomplete/person/?q=';
...
class EntityAcProvider {
	...
	createSnippet(type, suggestion){
		let theid = suggestion.id.match(/\d+/g).slice(-1)[0];
		let snippet = "<rs type='"+type+"' ref='#mpr"+theid+"'>"+suggestion.name+"$1</rs>$2"
		return snippet
	}
	...
	inflateSuggestion(replacementPrefix, suggestion) {
        console.log(entity)
		return {
			displayText: suggestion.name + ' (' + suggestion.id + ')',
